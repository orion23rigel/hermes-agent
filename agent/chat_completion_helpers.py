    ...
    def _call():
        try:
            ...
            if isinstance(e, ProviderRequestStalledError):
                ...
                return
            with request_terminal_lock:
                # An interrupt may race with the worker's transport exception:
                # force-closing the request socket is itself expected to make
                # the provider client raise.  Cancellation must win that race,
                # otherwise the resulting network error masks the user
                # interrupt as a provider failure (#6600).
                if agent._interrupt_requested:
                    _request_cancelled["value"] = True
                    request_monitor.cancel(
                        InterruptedError("provider request interrupted")
