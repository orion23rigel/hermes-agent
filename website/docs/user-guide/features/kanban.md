...
| `retryable_failure` | `{pid, claimer, exit_kind, exit_code}` | Worker exited with status `74` after a structured provider-request stall. The run is persisted as `retryable_failure`; below the failure limit the task returns to `ready`, while exhaustion blocks it. The ordinary bounded failure budget advances, and the outcome is reported separately from generic crashes. |
...
| `respawn_guarded` | `{reason}` | Dispatcher refused to re-spawn this ready task this tick. Reasons include `retryable_failure_cooldown` (fixed 60-second pause after a provider stall), `blocker_auth` (quota/auth/429), `recent_success`, and `active_pr`. The task stays in `ready`; a later tick gets another chance. Retryable provider stalls still consume the normal `consecutive_failures` budget and eventually auto-block via `gave_up`. |
