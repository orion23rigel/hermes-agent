...
| `HERMES_API_TIMEOUT` | Absolute wall-clock deadline for one physical LLM provider attempt in seconds (default: `1800`). Applies to streaming and non-streaming requests; progress never resets it. Provider/model `request_timeout_seconds` overrides it. A provider SDK may separately enforce a shorter transport timeout (native Anthropic defaults to `900`). |
...
| `HERMES_STREAM_STALE_TIMEOUT` | Stale stream inactivity timeout in seconds (default: `180`). When left implicit, local providers use the finite `HERMES_LOCAL_STREAM_STALE_TIMEOUT` default instead. Meaningful chunks reset this detector but never reset the absolute provider-attempt deadline. |
