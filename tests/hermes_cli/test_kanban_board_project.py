...
def _make_repo(path: Path) -> Path:
    """A project's primary path must be a real repo.

    ``create_task`` promotes a project-scoped task to ``workspace_kind
    ="worktree"`` and validates the owning repo anchor up front, so a bare
    tmp dir is rejected before the row is ever inserted.
    """
