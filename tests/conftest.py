...
def _live_system_guard(request, monkeypatch):
    ...
    test_pid = _os.getpid()
    # Resolve ownership lazily only when a test actually sends a signal.
    # Eagerly walking the whole process tree here made every test setup cost
    # seconds on hosts with large process tables, pushing large files beyond
    # the runner's timeout even when they contained only fast unit tests.
    # _is_own_subtree() below follows the target PID's parent chain, which is
    # both cheaper and sufficient to prove that a live target belongs to this
    # pytest process.
    try:
