...
def _kill_process_group(proc, escalate: bool = False):
    ...
    # Popen uses start_new_session=True on POSIX, so the child is its process
    # group leader. Signal that group immediately before doing comparatively
    # slow psutil tree discovery; otherwise a short-lived child can finish and
    # emit output while an interrupt is already being handled.
    try:
        if _IS_WINDOWS:
            proc.terminate()
        else:
